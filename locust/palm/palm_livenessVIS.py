...
class PalmProcess(PalmImageConfig):
    ...
    def register_palm_requestVIS(self):
        ...
        return self.client.post("/palm/vis/liveness", data=self.binary_data, params=global_config.get_queries(True),
                                headers=global_config.headers)

    @task
    def register_palm(self):
        res = self.register_palm_requestVIS()
        global_config.liveness_response(res, "palm livenessVIS")
